  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <label style={{ fontSize: 14, fontWeight: 600, color: '#BFDBFE' }}>{label}</label>
      ...
      <div style={{ position: 'relative', height: 6, borderRadius: 6, background: 'rgba(29,78,216,0.14)' }}>
        ...
        />
        <div style={{ position: 'absolute', top: '50%', transform: 'translateY(-50%)', left: `${((value - min) / (max - min)) * 100}%`, width: 18, height: 18, borderRadius: '50%', background: '#1D4ED8', border: '3px solid #F8FAFC', boxShadow: '0 2px 8px rgba(29,78,216,0.50)', marginLeft: -9, pointerEvents: 'none' }} />
      </div>
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <span style={{ fontSize: 11, color: 'rgba(147,197,253,0.40)' }}>{format(min)}</span>
        <span style={{ fontSize: 11, color: 'rgba(147,197,253,0.40)' }}>{format(max)}</span>
      </div>
    </div>
  )
}
...
function ResultCard({ icon: Icon, color, label, value, sub }) {
  return (
    <div style={{ padding: '24px', borderRadius: 16, background: `${color}0d`, border: `1px solid ${color}28`, display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      <div style={{ fontSize: 32, fontWeight: 900, color: '#F8FAFC', lineHeight: 1 }}>{value}</div>
      <div style={{ fontSize: 13, fontWeight: 700, color: '#BFDBFE' }}>{label}</div>
      {sub && <div style={{ fontSize: 12, color: 'rgba(147,197,253,0.48)', lineHeight: 1.5 }}>{sub}</div>}
    </div>
  )
}
...
export default function Calculator() {
  ...
  return (
    <PageWrapper>
      {/* Hero */}
      <section className="relative pt-28 pb-10 overflow-hidden" style={{ background: 'linear-gradient(180deg, #080809 0%, #111113 100%)' }}>
      ...
      {/* Calculator */}
      <section className="section-padding" style={{ background: '#111113' }} ref={ref}>
        <div className="container-wide">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
            ...
            <motion.div
              ...
              style={{ padding: '36px', borderRadius: 20, background: 'linear-gradient(145deg, rgba(28,28,32,0.85) 0%, rgba(17,17,19,0.95) 100%)', border: '1px solid rgba(255,255,255,0.08)', display: 'flex', flexDirection: 'column', gap: 32 }}
            >
              <div>
                <h2 style={{ fontSize: 20, fontWeight: 800, color: '#F8FAFC', marginBottom: 4 }}>Your Business</h2>
                <p style={{ fontSize: 13, color: 'rgba(147,197,253,0.52)' }}>Tell us about your current situation</p>
              </div>
              ...
              <div style={{ padding: '16px 20px', borderRadius: 12, background: 'rgba(29,78,216,0.06)', border: '1px solid rgba(29,78,216,0.18)' }}>
                ...
                <div style={{ fontSize: 22, fontWeight: 900, color: '#F8FAFC' }}>$599<span style={{ fontSize: 13, fontWeight: 500, color: 'rgba(147,197,253,0.50)' }}>/month</span></div>
                <div style={{ fontSize: 12, color: 'rgba(147,197,253,0.48)', marginTop: 4 }}>Bookkeeping + Tax + Payroll + Dedicated CPA</div>
              </div>
            </motion.div>
            ...
            >
              <div>
                <h2 style={{ fontSize: 20, fontWeight: 800, color: '#F8FAFC', marginBottom: 4 }}>Your Savings</h2>
                <p style={{ fontSize: 13, color: 'rgba(147,197,253,0.52)' }}>Updated instantly as you move the sliders</p>
              </div>
              ...
              >
                <p style={{ fontSize: 13, color: 'rgba(147,197,253,0.58)', marginBottom: 8 }}>Your estimated first-year ROI with Vanguard</p>
                ...
                <p style={{ fontSize: 12, color: 'rgba(147,197,253,0.42)', marginTop: 6 }}>in time and cost savings</p>
              </motion.div>
